...
function CircleThreader(svg) {
    var _svg = svg,
        ...
        _onPointerMove = (evt) => {
            ...
            var deltaX = evt.clientX - _pointerCoords.x,
                deltaY = evt.clientY - _pointerCoords.y;
        ...
        _getRandomColor = () => {
            var r = _getRandomColorComponent(),
                g = _getRandomColorComponent(),
                b = _getRandomColorComponent();
            return `#${r}${g}${b}`;
        },
        _calculateRadius = () => {
            _radius = Math.min(_height, _width) / 2 - PADDING;
        ...
        _setBouncingCirclePosition = (newPosition) => {
            ...
            if (length >= _radius) {
                ...
                distanceToCircle.multiply(
                    new Victor(_radius - _bouncingCircle.radius, _radius - _bouncingCircle.radius)
                );
                newPosition.add(distanceToCircle);

                // Draw thread.
                var thread = new Thread(_svg, {
        ...
        _bounceCirle = () => {
            var now = Date.now(),
                timeElapsed = now - _lastFrameTime;
        ...
        _cleanUp = () => {
            _bouncingCircle.svgElement.removeEventListener(
                'pointerdown',
                _bouncingCirclePointerDown
            );
            _removeAuxillaryEvents();
            cancelAnimationFrame(_frameId);
            cancelAnimationFrame(_pauseFrameId);
